...
const static vsi_nn_postprocess_map_element_t *postprocess_map_data = NULL;
const static vsi_nn_postprocess_map_element_t *postprocess_map_nb = NULL;
...
vsi_status vnn_PostProcessYolov3TinyUint8(struct Vnn_Wrapper *vnn_wrapper, detected_list *list)
{
	...
	status = show_result(vnn_wrapper, vsi_nn_GetTensor(graph, graph->output.tensors[0]), list);
	//status = show_result(graph, vsi_nn_GetTensor(graph, graph->output.tensors[0]));
...
const vsi_nn_postprocess_map_element_t *vnn_GetPostProcessMap(data_file_type data_type)
{
	if (data_type == FILE_DATA)
		return postprocess_map_data;
	else if (data_type == FILE_NB)
		return postprocess_map_nb;
	else
		return NULL;
}
...
uint32_t vnn_GetPostProcessMapCount(data_file_type data_type)
{
	if (data_type == FILE_DATA)
	{
		if (postprocess_map_data == NULL)
			return 0;
		else
			return sizeof(postprocess_map_data) / sizeof(vsi_nn_postprocess_map_element_t);
	}
	else if (data_type == FILE_NB)
	{
		if (postprocess_map_nb == NULL)
			return 0;
		else
			return sizeof(postprocess_map_nb) / sizeof(vsi_nn_postprocess_map_element_t);
	}
	else
	{
		return 0;
