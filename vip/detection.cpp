...
const char *coco_names[] = {"explosion"
};//class names
...
detection* make_network_boxes(vector<layer> layers_params,float thresh,int* num)
{
    ...
    if(num) *num = nboxes;
    detection *dets = (detection*)calloc(nboxes,sizeof(detection));//分配5670个detection
    for(i=0;i<nboxes;++i){
        dets[i].prob = (float*)calloc(l.classes,sizeof(float));//TODO:这里存在内存分配失败
		if (dets[i].prob == NULL) {
        fprintf(stderr, "Error: Memory allocation failed for dets[%d].prob\n", i);
        // 进行适当的错误处理，例如释放已分配的内存并退出
        for (int j = 0; j < i; ++j) {
            free(dets[j].prob);
        }
        free(dets);
    ...
    for(i=0;i<l.w*l.h;++i){
        ...
        for(n = 0;n<l.n;++n){           
            ...
            for(j=0;j<l.classes;++j){
                ...
                dets[count].prob[j] = (prob > thresh) ? prob : 0; //这里似乎出现了问题,有越界
				//找到问题了在j=0的适合,dets[4471].prob = NULL
            }
            ++count;
        }
    }
	...
	//{
		...
		for (int i=0; i<3; i++){
			...
			if(network==4||network==5){
				...
				if( 0 == i ) temp_blob.data = (float *)vnn_wrapper->output_data[2];//这里出现了问题,直接用这里的数据会出错
				else if( 1 == i ) temp_blob.data = (float *)vnn_wrapper->output_data[1];
				else if( 2 == i ) temp_blob.data = (float *)vnn_wrapper->output_data[0];
				blobs.push_back(temp_blob);
			}else if(network==3){
				blob temp_blob = {0, 0, NULL};
				if( 0 == i ) temp_blob.data = (float *)vnn_wrapper->output_data[0];
				else if( 1 == i ) temp_blob.data = (float *)vnn_wrapper->output_data[1];
				else if( 2 == i ) temp_blob.data = (float *)vnn_wrapper->output_data[2];
				blobs.push_back(temp_blob);
			}
#else
			if(network==4||network==5){
...
//		vsi_nn_tensor_t *input_tensor = NULL;
//		input_tensor = vsi_nn_GetTensor( vnn_wrapper->graph, vnn_wrapper->graph->input.tensors[0]);
//		int max_side = input_tensor->attr.size[0];    // image width
//		int min_side = input_tensor->attr.size[1];    // image height
		...
		//{
			sz1 = 1;
			output1_tensor = vsi_nn_GetTensor(vnn_wrapper->graph, vnn_wrapper->graph->output.tensors[0]);
			for(i = 0; i < output1_tensor->attr.dim_num; i++)
			{
				sz1 *= output1_tensor->attr.size[i];
			}
			stride = vsi_nn_TypeGetBytes(output1_tensor->attr.dtype.vx_type);
			output1_tensor_data = (uint8_t *)vsi_nn_ConvertTensorToData(vnn_wrapper->graph, output1_tensor);
			output1_buffer = (float *)malloc(sizeof(float) * sz1);
			for(i = 0; i < sz1; i++)
			{
				status = vsi_nn_DtypeToFloat32(&output1_tensor_data[stride * i], &output1_buffer[i], &output1_tensor->attr.dtype);
			...
			// output2 tensor
			sz2 = 1;
			output2_tensor = vsi_nn_GetTensor(vnn_wrapper->graph, vnn_wrapper->graph->output.tensors[1]);
			for(i = 0; i < output2_tensor->attr.dim_num; i++)
			{
			   sz2 *= output2_tensor->attr.size[i];
			}
			stride = vsi_nn_TypeGetBytes(output2_tensor->attr.dtype.vx_type);
			output2_tensor_data = (uint8_t *)vsi_nn_ConvertTensorToData(vnn_wrapper->graph, output2_tensor);
			output2_buffer = (float *)malloc(sizeof(float) * sz2);
			for(i = 0; i < sz2; i++)
			{
			   status = vsi_nn_DtypeToFloat32(&output2_tensor_data[stride * i], &output2_buffer[i], &output2_tensor->attr.dtype);
		...
		for(int i=0;i< nboxes;++i)
		{
			...
			if(cls >= 0){
				...
				int left  = (b.x-b.w/2.)*real_img_w; //img_w;
				int right = (b.x+b.w/2.)*real_img_w;  //img_w;
				int top   = (b.y-b.h/2.)*real_img_h; //img_h;
				int bot   = (b.y+b.h/2.)*real_img_h; //img_h;

				if( left < 0) left = 0;
				if( right > real_img_w -1) right = real_img_w - 1;
				if( top < 0 ) top = 0;
				if( bot > real_img_h - 1) bot = real_img_h - 1;

				sprintf(g_detect_objs.name[detect_num], "%s: %.0f%%", coco_names[cls], dets[i].prob[cls]*100);
				//printf("%s %f %f %f %f\n",g_detect_objs.name[detect_num],b.x*img_w,b.y*img_h,b.w*img_w,b.h*img_h);
				printf("%s %f %f %f %f\n",g_detect_objs.name[detect_num],b.x*real_img_w,b.y*real_img_h,b.w*real_img_w, b.h*real_img_h);
				g_detect_objs.pt1[detect_num] = cvPoint(left, top);
				g_detect_objs.pt2[detect_num] = cvPoint(right, bot);
				detect_num++;
			}
