...
int32_t roi_begin[3] = {0, 0, 3};
vsi_nn_preprocess_crop_t crop_for_norm_tensor_3 = {roi_begin, roi_size, 3};
...
vsi_nn_preprocess_base_t pre_process_for_norm_tensor_3[] =
    ...
    {VSI_NN_PREPROCESS_CROP, &crop_for_norm_tensor_3},
...
const static vsi_nn_preprocess_map_element_t preprocess_map_data[] =
{ 
{0, pre_process_for_norm_tensor_3, sizeof(pre_process_for_norm_tensor_3) / sizeof(vsi_nn_preprocess_base_t)},
};

const static vsi_nn_preprocess_map_element_t *preprocess_map_nb = NULL;
...
/*-------------------------------------------
                  Functions
-------------------------------------------*/
#define DATA_INPUT_META_NUM 1
#define NB_INPUT_META_NUM 2
static vnn_input_meta_t data_input_meta_tab[DATA_INPUT_META_NUM];
static vnn_input_meta_t nb_input_meta_tab[NB_INPUT_META_NUM];
static void _load_input_meta(data_file_type data_type)
{
    uint32_t i;
    if(data_type==FILE_DATA) {

        for (i = 0; i < DATA_INPUT_META_NUM; i++) {
            memset(&data_input_meta_tab[i].image.preprocess,
                   VNN_PREPRO_NONE, sizeof(int32_t) * VNN_PREPRO_NUM);
        }
        if (vnn_UseImagePreprocessNode()) {
            /* lid: images_328 */
            data_input_meta_tab[0].image.preprocess[0] = VNN_PREPRO_NONE;
            data_input_meta_tab[0].image.preprocess[1] = VNN_PREPRO_NONE;
            data_input_meta_tab[0].image.preprocess[2] = VNN_PREPRO_NONE;

        } else {
            /* lid: images_328 */
            data_input_meta_tab[0].image.preprocess[0] = VNN_PREPRO_REORDER;
            data_input_meta_tab[0].image.preprocess[1] = VNN_PREPRO_MEAN;
            data_input_meta_tab[0].image.preprocess[2] = VNN_PREPRO_SCALE;
            data_input_meta_tab[0].image.reorder[0] = 0;
            data_input_meta_tab[0].image.reorder[1] = 1;
            data_input_meta_tab[0].image.reorder[2] = 2;
            data_input_meta_tab[0].image.mean[0] = 0;
            data_input_meta_tab[0].image.mean[1] = 0;
            data_input_meta_tab[0].image.mean[2] = 0;
            data_input_meta_tab[0].image.scale = 0.003921569;

        }
    }
    else if(data_type==FILE_NB){
        for (i = 0; i < NB_INPUT_META_NUM; i++)
        {
            memset(&nb_input_meta_tab[i].image.preprocess,
                   VNN_PREPRO_NONE, sizeof(int32_t) * VNN_PREPRO_NUM);
        }
        /* lid: images_328_0 */
        nb_input_meta_tab[0].image.preprocess[0] = VNN_PREPRO_NONE;
        nb_input_meta_tab[0].image.preprocess[1] = VNN_PREPRO_NONE;
        nb_input_meta_tab[0].image.preprocess[2] = VNN_PREPRO_NONE;
        nb_input_meta_tab[0].image.reorder[0] = 0;
        nb_input_meta_tab[0].image.mean[0] = 0;
        nb_input_meta_tab[0].image.mean[1] = 0;
        nb_input_meta_tab[0].image.mean[2] = 0;
        nb_input_meta_tab[0].image.scale = 0.003921569;

        /* lid: images_328_1 */
        nb_input_meta_tab[1].image.preprocess[0] = VNN_PREPRO_NONE;
        nb_input_meta_tab[1].image.preprocess[1] = VNN_PREPRO_NONE;
        nb_input_meta_tab[1].image.preprocess[2] = VNN_PREPRO_NONE;
        nb_input_meta_tab[1].image.reorder[0] = 0;
        nb_input_meta_tab[1].image.mean[0] = 0;
        nb_input_meta_tab[1].image.mean[1] = 0;
        nb_input_meta_tab[1].image.mean[2] = 0;
        nb_input_meta_tab[1].image.scale = 0.003921569;
    }

}
void load_input_meta(data_file_type data_type)
{
    _load_input_meta(data_type);
}
    ...
    )
{
    ...
    uint32_t *reorder;
    if(data_type==FILE_DATA) {
        scale = data_input_meta_tab[0].image.scale;
        mean_value = data_input_meta_tab[0].image.mean;
        reorder = data_input_meta_tab[0].image.reorder;
    }
    else if(data_type==FILE_NB){
        scale = nb_input_meta_tab[0].image.scale;
        mean_value = nb_input_meta_tab[0].image.mean;
        reorder = nb_input_meta_tab[0].image.reorder;
    }
    ...
    )
{
    ...
    if(data_type == FILE_DATA){
		meta = data_input_meta_tab[idx];
    }else if(data_type == FILE_NB){
		meta = nb_input_meta_tab[idx];
    }
    ...
    )
{
    ...
    {
        printf("vnn_PreProcessYolov3TinyUint8_ImageProcess Only support JPG image file\n");
        status = VSI_FAILURE;
        TEST_CHECK_STATUS(status, final);
    }
    ...
    )
{
    ...
    {
        status = _handle_multiple_inputs(graph, i, inputs[i], data_type);
        TEST_CHECK_STATUS(status, final);
    }
...
{
	if(data_type == FILE_DATA)
		return preprocess_map_data;
	else if(data_type == FILE_NB)
    	return preprocess_map_nb;
    else
    	return NULL;
}
...
{
	if(data_type == FILE_DATA){
	    if (preprocess_map_data == NULL)
        	return 0;
	    else
	        return sizeof(preprocess_map_data) / sizeof(vsi_nn_preprocess_map_element_t);
	}
	else if(data_type == FILE_NB){
		if (preprocess_map_nb == NULL)
        return 0;
    else
			return sizeof(preprocess_map_nb) / sizeof(vsi_nn_preprocess_map_element_t);
	}else{
		return 0;
	}
}
...
int set_norm_tensor_0_format(uint8_t format)
{
	...
	switch(format){
	case 0:
		source_format_for_norm_tensor_3 = VSI_NN_SOURCE_FORMAT_IMAGE_NV12;
		break;
	case 1:
		source_format_for_norm_tensor_3 = VSI_NN_SOURCE_FORMAT_IMAGE_RGB;
		break;
	default:
		ret = -1;
		break;
...
{
	size_for_norm_tensor_3.w = w;
	size_for_norm_tensor_3.h = h;
	size_for_norm_tensor_3.c = c;
}
